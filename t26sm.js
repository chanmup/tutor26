$(document).ready( function(){
    ...
    var clipboard = new ClipboardJS('.t26_tool_copy');
...
function t26_tool_submit(b) {
    var btn = $(b);
    var container = btn.closest("div.t26_tool_container");
    var textarea = container.find("textarea.t26_tool_text");
    var formatter = textarea.attr("data-formatter");
    var ajaxurl = $('#t26_tool_admin_url').val();
    ...
    if(formatter == "base64-encoder") {
        container.find(".t26_tool_result").val(btoa(val));
        document.getElementsByClassName("output_card")[0].scrollIntoView(false);
        toastr.success('done');
        return true;
    }
    ...
    if(formatter == "base64-decoder") {
        container.find(".t26_tool_result").val(atob(val));
        document.getElementsByClassName("output_card")[0].scrollIntoView(false);
        toastr.success('done');
        return true;
    }
    ...
    if(formatter == 'sorting-list'){
        ...
        container.find(".t26_tool_result").val(output.join("\n"));
        document.getElementsByClassName("output_card")[0].scrollIntoView(false);
    ...
    if(formatter == 'string-utils'){
        ...
        container.find(".t26_tool_result").val(output);
        document.getElementsByClassName("output_card")[0].scrollIntoView(false);
    ...
    $.ajax({
        ...
        data: data,
        success: function (response) {
            container.find(".t26_tool_result").val(response);
            document.getElementsByClassName("output_card")[0].scrollIntoView(false);
            toastr.success('done');        
        }
    });
...
function codemirror_copy(b) {
    ...
    new ClipboardJS('.t26_tool_codemirror_copy', {
